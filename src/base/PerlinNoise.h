...
public:
	...
	double noise(double x, double y, double z = 0.f) {
	...
	// generate nose map of size w*h in a 2d array
	std::vector<std::vector<float>> Generate2DnoiseMap(int width, int height, float scale, int octaves, float persistance, float lacunarity, float Xoffset, float Yoffset) {
	...
	// generate nose map of size w*h in a 2d array
	std::vector<std::vector<float>> GenerateGradientTrick2DnoiseMap(int width, int height, float scale, int octaves, float persistance, float lacunarity, float Xoffset, float Yoffset, float gradientFactor, float heightMultiplier = 1, std::vector<std::vector<glm::vec2>> chunckMap = {}, int chunckMapFactor = 1) {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				...
				for (int i = 0; i < octaves; i++) {
					float sampleX = ((float)x + Xoffset - halfWidth + (float)octavesOffsets[i].x) / scale * frequency;
				...
				noiseMap[x][y] = heightMultiplier * noiseHeight;
			}
		}
