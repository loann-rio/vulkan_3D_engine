...
void ObjectManager::startLoadModel(DescriptorPool& pool)
{
    ...
    /*std::shared_ptr<Model> plane = createPlane(device, 10, 10, {0, 0, 0}, "textures/emptyTexture.jpg");
    ...
    gltfTransform.rotation = { -pi<float>, 0.70f, 0 };
    ...
    auto plane1 = GameObjectFactory::createGameObject<GameObjectModel>(device);
    plane1->setModel(plane); 
    //plane1->transform.rotation.z = -pi<float> / 2;
    //plane1->transform.translation.x = 10.f;
    //plane1->transform.translation.y = 1.f;
    plane1->createDescriptorSet(pool); 
