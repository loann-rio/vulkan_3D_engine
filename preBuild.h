...
static std::shared_ptr<Model> create_terrain(Device& device, const int detail, const float sizePlane, glm::vec3 color)
{
    ...
    return std::make_unique<Model>(device, modelBuilder, "textures/nullTexture.png");
}
...
static std::unique_ptr<Model> createPlane(Device& device, const unsigned int detail, const float sizePlane, glm::vec3 color) {
    ...
    for (unsigned int i = 0; i < detail + 1; i++) {
        for (unsigned int j = 0; j < detail + 1; j++)
        {
            ...
            modelBuilder.vertices.push_back({ {i * sizePlane / detail, 0.f, j * sizePlane / detail}, {0, 0, 0}, {0, -1, 0}, {(float) (i) / (float) detail , (float) (j) / (float) detail } });
