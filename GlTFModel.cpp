...
void GlTFModel::TextureModel::fromglTfImage(Device& device, tinygltf::Image& gltfimage, std::string path, VkQueue copyQueue)
{
	...
	if (isKtx2) {
		...
		Texture texture1{ device, buffer, width, height, bufferSize, mipLevels }; 
...
void GlTFModel::TextureModel::destroy()
{
	delete texture;
}
...
GlTFModel::Primitive::Primitive(uint32_t firstIndex, uint32_t indexCount, uint32_t vertexCount, GlTFModel::Material& material) : firstIndex(firstIndex), indexCount(indexCount), vertexCount(vertexCount), material(material) {
...
void GlTFModel::Model::loadNode(Node* parent, const tinygltf::Node& node, uint32_t nodeIndex, const tinygltf::Model& model, LoaderInfo& loaderInfo, float globalscale)
{
...
void GlTFModel::Model::loadTextures(tinygltf::Model& gltfModel, Device& device, VkQueue transferQueue)
{
...
void GlTFModel::Model::loadFromFile(std::string filename, Device& device, VkQueue transferQueue, float scale)
{
