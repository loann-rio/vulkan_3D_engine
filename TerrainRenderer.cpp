...
TerrainRenderer::TerrainRenderer(Device& device, VkRenderPass renderPass, VkDescriptorSetLayout globalSetLayout) : device{ device }
{
...
void TerrainRenderer::createPipelineLayout(std::vector<VkDescriptorSetLayout> descriptorSetLayout)
{
...
void TerrainRenderer::createPipeline(VkRenderPass renderPass)
{
	...
	pipeline = std::make_unique<Pipeline>(
		device,
		"terrainShader.vert.spv",
		"terrainShader.frag.spv",
		pipelineConfig
	);
}
...
void TerrainRenderer::renderTerrain(FrameInfo& frameInfo, GameObject::Map& chunks)
{
	...
	for (auto& kv : chunks)
	{
		...
		vkCmdBindDescriptorSets(
			...
			pipelineLayout,
			1, 1,
			&obj.model->descriptorSet[frameInfo.frameIndex],
			0,
			nullptr
		);
