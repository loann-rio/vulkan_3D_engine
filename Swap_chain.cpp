...
Swap_chain::Swap_chain(std::shared_ptr<Device> deviceRef, VkExtent2D windowExtent)
    : device{ deviceRef }, windowExtent{ windowExtent } {
    init();
}

Swap_chain::Swap_chain(std::shared_ptr<Device> deviceRef, VkExtent2D windowExtent, std::shared_ptr<Swap_chain> previous)
