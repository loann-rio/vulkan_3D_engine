...
public:
    ...
    void sendMousePosition(glm::vec2 mousePos, const glm::mat4& viewMatrix, const glm::mat4& projectionMatrix);

    void setIDObjects(GameObject::id_t link1, GameObject::id_t link2, GameObject::id_t link3) {
...
private:
    ...
    void setNewPos(std::shared_ptr<GameObject::Map> gameObjects, float angle1, float angle2, float angle3);

    std::vector<GameObject::id_t> links{};
    const std::vector<float> sizeLinks = { .55f, 1.8f, 1.7f };
    const glm::vec3 basePos = { 7.f, -0.55f, 7.f };
};
